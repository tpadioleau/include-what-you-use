...
class IwyuBaseAstVisitor : public BaseAstVisitor<Derived> {
 public:
  ...
  bool VisitInitListExpr(InitListExpr* expr) {
    ...
    // If an InitListExpr is implicitly generated by the compiler,
    // it initializes a field of another aggregate, and the type should already
    // be complete.
    if (expr->isExplicit() && expr->isPRValue()) {
      // Otherwise, report the initialized type. This makes sense even
      // in the case of a subaggregate because this allows adding const
      // reference to its type. 'isPRValue' means that this is not just passing
      // a reference ('isTransparent()' check could probably be used instead).
      ReportTypeUse(CurrentLoc(), expr->getType().getTypePtr(),
                    DerefKind::None);
    }
    return true;
  }
